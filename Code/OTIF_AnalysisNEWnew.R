...
featuresToAnalyze <- c("OTIF.Bin", "OTIF.Scope","Doc.Type","SoldTo.Industry.L1","Plant","SoldTo.Profitability.Segment","Request.Qty.M3","Early.Bin"
...
rf_fit <- caret::train(OTIF.Bin ~ ., 
                data = data=na.omit(DATA[, featuresToAnalyze, with=F]), 
                method = "ranger")

rfee <- caret::rfe(x = na.omit(head(DATA[, featuresToAnalyze, with=F], n=10000)), y= OTIF.Bin)
