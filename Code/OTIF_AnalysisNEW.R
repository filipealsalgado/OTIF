...
OTIF_dataC[is.na(Unload.Date.OT)]$Unload.Date.OT               <- as.factor('NA')
...
#OTIF_dataC$Request.Unload_BusinessDays       <- as.factor(OTIF_dataC$Request.Unload_BusinessDays)
#OTIF_dataC$Created.Unload_BusinessDays       <- as.factor(OTIF_dataC$Created.Unload_BusinessDays)
#OTIF_dataC$X1stConfirmed.Unload_BusinessDays <- as.factor(OTIF_dataC$X1stConfirmed.Unload_BusinessDays)
#OTIF_dataC$ReadyPlanning.Unload_BusinessDays <- as.factor(OTIF_dataC$ReadyPlanning.Unload_BusinessDays)
#OTIF_dataC$Date.ReadyPlanning_WeekDay        <- as.factor(OTIF_dataC$Date.ReadyPlanning_WeekDay)
...
any(is.na(OTIF_dataC$SoldTo.Sales.Area)) 
any(is.na(OTIF_dataC$SoldTo.ReportingGroup)) 
...
any(is.na(OTIF_dataC$Unload.Date.OT)) #TRUE 9885
...
any(is.na(OTIF_dataC$Doc.PackingType)) 
...
any(is.na(OTIF_dataC$Date.ReadyPlanning)) #TRUE 201872
any(is.na(OTIF_dataC$Avg.Planning.Time)) #TREUE 201872
any(is.na(OTIF_dataC$DelayReason))

any(OTIF_dataC$OTIF.Bin=='')
any(OTIF_dataC$OTIF.Scope=='')
any(OTIF_dataC$Doc.Type=='')
any(OTIF_dataC$SoldTo.Industry.L1=='')
any(OTIF_dataC$Plant=='')
any(OTIF_dataC$SoldTo.Sales.Area=='')
any(OTIF_dataC$SoldTo.ReportingGroup=='') 
any(OTIF_dataC$Order.Doc=='')
any(OTIF_dataC$Order.Item=='')
any(OTIF_dataC$Sales.Org.Code=='')
any(OTIF_dataC$SoldTo.Profitability.Segment=='')
any(OTIF_dataC$Request.Qty.M3=='')
any(OTIF_dataC$Request.Qty.UoM=='')
any(OTIF_dataC$Early.Bin=='')
any(OTIF_dataC$Grid.Level.1=='')
any(OTIF_dataC$Production.OTIF.Bin=='')
any(OTIF_dataC$Product.Allocation.Qty.on.MAD..M3=='')
any(OTIF_dataC$SoldTo=='')
any(OTIF_dataC$ShipTo.Country=='')
any(OTIF_dataC$Shipping.Condition=='')
any(OTIF_dataC$GFFTT=='')
any(OTIF_dataC$Material=='')
any(OTIF_dataC$Request.Date=='')
any(OTIF_dataC$Created.Date=='')
any(OTIF_dataC$X1st.Confirmed.Date=='')
any(OTIF_dataC$Unload.Date.OT=='') 
any(OTIF_dataC$Delivery.Priority=='')
any(OTIF_dataC$OTIF=='')
any(OTIF_dataC$Material.TopReference=='')
any(OTIF_dataC$Material.TopFinish=='')
any(OTIF_dataC$Material.Quality=='')
any(OTIF_dataC$Order.DeliveryBlock=='')
any(OTIF_dataC$Doc.PackingType=='')
any(OTIF_dataC$Year=='')
any(OTIF_dataC$Month=='')
any(OTIF_dataC$Time.Week=='')
any(OTIF_dataC$Week=='')
any(OTIF_dataC$Date.Type=='')
any(OTIF_dataC$Date.ReadyPlanning=='') # Delete NA 
any(OTIF_dataC$Avg.Planning.Time=='')
any(OTIF_dataC$DelayReason=='')



OTIF_dataC <- OTIF_dataC[OTIF.Bin != 'NA']
OTIF_dataC[OTIF.Bin == '1']$OTIF.Bin <- as.factor(1)
OTIF_dataC[OTIF.Bin == '0']$OTIF.Bin <- as.factor(0)
OTIF_teste <- OTIF_dataC[!(is.na(OTIF_dataC$Date.ReadyPlanning)) & !(is.na(OTIF_dataC$Avg.Planning.Time)) & !(is.na(OTIF_dataC$Unload.Date.OT))]

OTIFtest50 <- head(OTIF_teste, n=50000)
library(mlbench)
library(caret)
set.seed(998)

modelFit <- caret::train( OTIF.Bin~.,data=OTIFtest50, method="rf", importance = TRUE )  
...
sample <- 
   sample.split(OTIF_dataC$OTIF.Bin, SplitRatio = 0.70) # SplitRatio = percent of sample==TRUE 

# Training Data 
train <- subset(OTIF_dataC, sample == TRUE) 


# Testing Data 
test <- subset(OTIF_dataC, sample == FALSE) 
...
e1071::naiveBayes(OTIF.Bin ~., data = train)
#Key-Nearest Neighbours
class::knn(train, test, cl, k = 1, l=0, prob = FALSE, use.all = TRUE)
#Logistic Regression
a <-stats::glm(OTIF.Bin ~., family = binomial(link = 'logit'), data = train)
